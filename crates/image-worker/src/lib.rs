...
//! the `image-wasm` crate (the browser build, which serves the same `Worker`
...
/// The catalog name DuckDB sees in `ATTACH 'img' (TYPE vgi, …)`. Defaults to
/// `img`, but honors an explicit override so a test harness can rename it. Also
