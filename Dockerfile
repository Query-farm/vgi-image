...
#
# Single image that serves every network transport of the `image` VGI worker:
...
#
# This worker is STATELESS: image decode / EXIF / perceptual-hashing /
# thumbnailing are pure functions of the input BLOB, with no per-attach state,
# no model registry, and therefore no /data volume and no
...
# runtime ships. The workspace depends only on crates.io (`vgi`, `image`,
# `kamadak-exif`, `image_hasher`), so a plain `cargo build` needs no extra
# system libraries in the build stage.
...
RUN --mount=type=cache,target=/usr/local/cargo/registry \
    ...
    cargo build --release --locked --bin image-worker \
    && cp target/release/image-worker /image-worker
...
ARG SOURCE_URL=https://github.com/Query-farm/vgi-image
...
LABEL org.opencontainers.image.title="vgi-image" \
      org.opencontainers.image.description="Image decode / EXIF / perceptual-hashing / thumbnailing / format conversion as a VGI worker for DuckDB/SQL (stdio + HTTP + TCP)" \
...
ENV PORT=8000 \
    ...
    VGI_IMAGE_GIT_COMMIT=${GIT_COMMIT}
...
COPY --from=build --chmod=0755 /image-worker /usr/local/bin/image-worker
