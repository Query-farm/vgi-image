#!/usr/bin/env bash
# Build the browser (`worker:`) build of the image VGI worker: image-wasm
...
LIB="target/$TARGET/release/libimage_wasm.a"
...
#
# OPT_LEVEL=3 overrides the workspace's release profile for this target only.
#
...
# thread-caching, so concurrent serve threads scale instead of convoying.
#
# STACK_SIZE / DEFAULT_PTHREAD_STACK_SIZE: emscripten defaults to a 64 KiB stack,
# which this worker overflows — the Arrow encoders and image codecs nest deeply,
# and an overflow surfaces as a bare "memory access out of bounds". The serve
